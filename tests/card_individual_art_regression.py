...
CARD_DATA_DIR = ROOT / "external" / "data" / "cards"
...
def json_cards() -> dict[str, dict]:
    rows = {}
    for path in CARD_DATA_DIR.glob("*.json"):
        with path.open(encoding="utf-8") as handle:
            properties = json.load(handle)["properties"]
        rows[properties["object_id"]] = properties
    return rows


def expected_art_path(card_id: str, manifest: dict) -> str:
    color_folder = manifest["card_color_ids"][card_id].removeprefix("color_")
...
def validate_card_configs(
    ...
    for card_id in card_ids:
        ...
        assert card_id in data_cards, f"missing card JSON: {card_id}"
        expected_path = expected_art_path(card_id, manifest)
        ...
        assert data_cards[card_id]["card_texture_path"] == expected_path, (
            f"unexpected card JSON art path for {card_id}"
        )
        expected_color = manifest["card_color_ids"][card_id]
        assert cards[card_id]["card_color_id"] == expected_color
        assert workbook_cards[card_id]["card_color_id"] == expected_color
        assert data_cards[card_id]["card_color_id"] == expected_color
...
def validate_card_art(
    ...
    with Image.open(image_path) as image:
        ...
        key_hex = key_color_for_card(
            manifest,
            card_id,
            manifest["card_color_ids"][card_id],
        )
...
def validate_manifest_invariants(manifest: dict) -> list[str]:
    ...
    validate_card_color_contract(manifest, phase_ids)
...
def validate_manifest() -> None:
    ...
    cards = csv_cards()
    data_cards = json_cards()
    active_ids = [card_id for card_id in phase_ids if card_id in data_cards]
    validate_card_configs(
        active_ids,
        cards,
        xlsx_cards(),
        data_cards,
...
def validate_cards(card_ids: list[str], manifest: dict) -> None:
    ...
    data_cards = json_cards()
    active_ids = [card_id for card_id in card_ids if card_id in data_cards]
    validate_card_configs(
        active_ids,
        cards,
        workbook_cards,
    ...
    for card_id in card_ids:
        validate_card_art(card_id, manifest)
