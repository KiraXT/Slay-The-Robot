...
**Goal:** 将费用文字整体右上移动 2 像素，并将卡名整体下移 2 像素。
...
- 费用区域必须从 `Rect2(2, 3, 38, 38)` 改为 `Rect2(4, 1, 38, 38)`。
- 卡名区域必须从 `Rect2(34, 6, 90, 24)` 改为 `Rect2(34, 8, 90, 24)`。
...
- Produces: 新的固定矩形 `CardName = Rect2(34, 8, 90, 24)`、`EnergyCost = Rect2(4, 1, 38, 38)`。
...
Expected: 报告卡名实际区域仍为 `Rect2(34, 6, 90, 24)`，费用实际区域仍为 `Rect2(2, 3, 38, 38)`。
...
视觉确认费用相对旧版本右上移动 2 像素，卡名相对旧版本下移 2 像素；长文本不溢出。随后执行：
