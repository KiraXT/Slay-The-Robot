...
**Goal:** 将费用文字从过度右移的位置向左回调 1 像素，并保持卡名位置不变。
...
- 费用区域必须从 `Rect2(6, 1, 38, 38)` 改为 `Rect2(5, 1, 38, 38)`。
...
- Produces: 固定矩形 `CardName = Rect2(34, 8, 90, 24)`、`EnergyCost = Rect2(5, 1, 38, 38)`。
...
_assert_exact_rect(visual, "EnergyCost", Rect2(5.0, 1.0, 38.0, 38.0), "energy cost")
```
...
Expected: 报告费用实际区域仍为 `Rect2(6, 1, 38, 38)`。
...
视觉确认费用相对提交 `d5a7d0c` 左移 1 像素、纵向位置不变，卡名位置不变；长文本不溢出。随后执行：
