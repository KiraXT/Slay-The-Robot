...
def validate_card_color_contract(
    manifest: dict,
    canonical_card_ids: list[str],
) -> dict[str, str]:
    card_color_ids = manifest.get("card_color_ids")
    assert isinstance(card_color_ids, dict), "card_color_ids must be an object"
    assert set(card_color_ids) == set(canonical_card_ids), (
        "card_color_ids must contain exactly one color per canonical card ID"
    )
    assert set(card_color_ids.values()) <= EXPECTED_COLOR_IDS, (
        "card_color_ids must use the six expected color IDs"
    )
    return card_color_ids
