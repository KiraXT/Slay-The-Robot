...
def render_contact_sheet(phase: str, output_path: Path) -> Path:
    ...
    for index, card_id in enumerate(card_ids):
        ...
        if card is None:
            raise KeyError(f"card is missing from cards.csv: {card_id}")
        texture_path = (card.get("card_texture_path") or "").strip()
        if not texture_path:
            raise ValueError(f"card has no texture path: {card_id}")
